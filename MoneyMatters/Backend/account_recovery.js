...
router.put("/recover/setpassword/:recid", async (req, res) => {
  try {
    ...
    if (exists.rows[0].code != code) {
      refresh();
      var attempts = await pool.query(
        "SELECT attempts FROM AccountRecovery WHERE accountrecoveryid = $1",
        [recid]
      );
      ...
      return res
        .status(400)
        .json("Error, wrong code. " + nbattempts + " attempts left.");
    }
    //Update password
    const usernameraw = await pool.query(
      "SELECT username FROM AccountRecovery WHERE AccountRecoveryId = $1",
      [recid]
    );
    ...
    //Delete request
    await pool.query(
      "DELETE FROM AccountRecovery WHERE AccountRecoveryId = $1",
      [recid]
    );
